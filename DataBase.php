<?php
/**
 * PDO Wrapper: Surcouche pour PDO - PHP 
 *
 * Copyright (c) 20010-2011 Kokolitolithe
 *
 * Distributed under the terms of the MIT License.
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright  2010-2011 Michael Henretty <michael.henretty@gmail.com>
 * @website    http://ref.moufbox.fr
 * @link       https://github.com/kokolitolithe-mvc/bdd-kokolitolithe
 */
...
class DataBase {
	 ...
	 * @param bool $timestamp_this (Optional) - if true we set created_at and updated_at values to now
	...
	public function update($table, $params, $wheres=array(), $timestamp_this=null) {
		...
		if ($timestamp_this === true) {
			$set_string .= ($add_comma ? ", " : "") . "updated_at='".date('Y-m-d H:i:s')."'";
		}
	...
	public function insert($table, $params = array(), $timestamp_this = null) {
		...
		// add the timestamp columns if neccessary
		if ($timestamp_this === true) {
			$columns_str .= ($add_comma ? ', ' : '') . '`created_at`, `updated_at`';
			$values_str .= ($add_comma ? ", " : "") ."'". date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."'";
		}
	 ...
	 * @param bool $timestamp_these (Optional), if true we set created_at and updated_at values to NOW() for each row
	 * @return mixed - new primary key of inserted table, false on failure
	 */
	public function insertMultiple($table, $columns = array(), $rows = array(), $timestamp_these = null) {
		...
		foreach ($rows as $row_index => $row_values) {
			...
			if ($timestamp_these) {
				$values_str .= ($add_comma_forvalue ? ", " : "") ."'". date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."'";
			}
			$values_str .= ')';
		}
